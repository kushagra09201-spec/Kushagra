...
### Situation
In recharge service platform, I was assigned a new requirement to store user specific data for future analysis. The initial approach was to get these bills from external service, then validate and format the data, and store it in database synchronously.
...
After further analysis, I recommended a different approach using Cassandra database, as it is suitable for high write throughput which aligned with our requirements. I also proposed an asynchronous event-driven approach using Kafka which can decouple the formatting and validation of the data. This reduced the response time of the service and made it lightweight.
...
After analyzing the risks and future maintenance efforts, I proposed not just migration but upgrading the codebase from Kotlin and Java 8 to Java 21 and Spring Boot 3, aligned with other services. I focused on delivery within the deadline while spending significant time for multiple framework migration challenges and ensuring no compatibility issues through iterative testing. Despite additional efforts, I have met the deadline
...
This task helped me understand the importance of looking beyond immediate solutions and balancing short-term delivery with long-term maintainability. Additionally, learnt an additional skill to convert kotlin code to upgraded java 21
...
This helped me understand the system and start working on feature development and deployment. Within a short period, I was independently able to resolve production issues as well.
